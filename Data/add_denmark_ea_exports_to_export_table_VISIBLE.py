#!/usr/bin/env python3
"""
Append visible Denmark -> Euro Area export rows to the `Export` table in the
...
3. Appends 16 visible rows at the bottom of `Eksport Tabel` / table `Export`:
...
def make_row_xml(row_number: int, row_values: list[object], shared: SharedStrings) -> str:
    ...
    # Keep the appended rows VISIBLE.
    # The workbook has an active filter on exp_country_code, and rows hidden by
    # that filter are stored with hidden="1". The previous version copied that
    # hidden state, which made the added DEN -> EA rows exist in the table but
    # appear invisible in Excel. DEN is already allowed by the active filter, so
    # new DEN -> EA rows should not be marked hidden.
    return (
        f'<row r="{row_number}" spans="1:9" x14ac:dyDescent="0.75">'
        + "".join(cells)
        + "</row>"
    )
...
def verify_output_rows(output_workbook: Path) -> tuple[int, int]:
    """Return (DEN->EA row count, hidden DEN->EA row count) in the Export table."""
    with zipfile.ZipFile(output_workbook, "r") as zin:
        shared = SharedStrings.from_zip(zin)
        _, export_sheet_path = first_existing_sheet_path(
            zin, ["Eksport Tabel", "Export Tabel", "Export table"]
        )
        export_table_path = table_path_from_sheet(zin, export_sheet_path, "Export")
        table_root = ET.fromstring(zin.read(export_table_path))
        ...
        sheet_root = ET.fromstring(zin.read(export_sheet_path))
        c1, r1, c2, r2 = parse_range(table_ref)
        rows_by_num = {int(row.get("r")): row for row in sheet_root.find(f"{M}sheetData").findall(f"{M}row")}
        start_col = col_to_index(c1)
        end_col = col_to_index(c2)

        header_cells = row_cells_by_col(rows_by_num[r1])
        headers = [shared.cell_text(header_cells.get(col)) for col in range(start_col, end_col + 1)]
        total = 0
        hidden = 0
        for r in range(r1 + 1, r2 + 1):
            row = rows_by_num.get(r)
            if row is None:
                continue
            cells = row_cells_by_col(row)
            vals = [shared.cell_text(cells.get(col)) for col in range(start_col, end_col + 1)]
            rec = dict(zip(headers, vals))
            if rec.get("exp_country_code") == "DEN" and rec.get("dest_country_code") == "EA":
                total += 1
                if row.get("hidden") == "1":
                    hidden += 1
        return total, hidden
...
def main() -> None:
    ...
    den_ea_count, hidden_den_ea_count = verify_output_rows(output_path)

    print(f"Created: {output_path}")
    print(f"Verified DEN -> EA rows in Export table: {den_ea_count}")
    print(f"Hidden DEN -> EA rows: {hidden_den_ea_count}")
